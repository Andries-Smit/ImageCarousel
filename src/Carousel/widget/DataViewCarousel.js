...
dojo.provide('Carousel.widget.DataViewCarousel');
...
__CarouselType = {
    ...
    setDataobject : function(dataobject) {
        var self = this;
        var path = (this.refentity + "/" + this.refcaptionattr ).split("/");
        
        var constraint  = "//" + path[1] + "[" + path[0] + " = " + dataobject.getGUID() + "]";
        this.captionattr = path[2];
        
        mx.data.get({
            xpath: constraint,
            filter : {
                sort : [[this.refsortattr, "asc"]] 
            },
...
mendix.widget.declare('Carousel.widget.DataViewCarousel', __CarouselType);
;;
